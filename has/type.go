...
func SameTypeAs(given interface{}) matcher.Matcher {
	return matcher.New(
...
func NotSameTypeAs(given interface{}) matcher.Matcher {
	return matcher.New(
		func(actual interface{}, chain matcher.Chain) matcher.Chain {
			return chain.
				Add(func() matcher.MatchResult {
					if actual == nil && given == nil {
						return matcher.Failed("want not same type as <nil>; got <nil>")
					}
					if reflect.TypeOf(actual) == reflect.TypeOf(given) {
						return matcher.Failed(
							fmt.Sprintf("want not same type as %[1]T; got %[2]T", actual, given))
					}
					return matcher.Matched()
				})
		},
